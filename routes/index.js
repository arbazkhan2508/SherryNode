...
router.get("/car/:id", async function (req, res) {
  if (req.isAuthenticated()) {
    let user = await userModel.findOne({ email: req.user.email });
    let car = await vehicleModel.findOne({ _id: req.params.id }).populate('ownerId') 
...
router.get("/bikes", async function (req, res, next) {
  if (req.isAuthenticated()) {
    var v = 'buy';
    let user = await userModel.findOne({ email: req.user.email });
    let allVehicles = await vehicleModel.find({ vehicleType: { $regex: /^bike$/i } }).limit(6).skip(0)
    res.render("All vehicles", { allVehicles: allVehicles, user: user,v:v });
  } else {
    var v = 'buy';
    let allVehicles = await vehicleModel.find({vehicleType: { $regex: /^bike$/i }  }).limit(6).skip(0)
    res.render("All vehicles", { allVehicles: allVehicles, user: null,v:v });
  }
});
...
router.get("/cars", async function (req, res, next) {
  if (req.isAuthenticated()) {
    var v = 'buy';
    let user = await userModel.findOne({ email: req.user.email });
    let allVehicles = await vehicleModel.find({ vehicleType: { $regex: /^car$/i } }).limit(6).skip(0)
    res.render("All vehicles", { allVehicles: allVehicles, user: user,v:v });
  } else {
    var v = 'buy';
    let allVehicles = await vehicleModel.find({vehicleType: { $regex: /^car$/i }  }).limit(6).skip(0)
...
router.get("/buyvehicles", async function (req, res, next) {
  if (req.isAuthenticated()) {
    var v = 'buy';
    let user = await userModel.findOne({ email: req.user.email });
    let allVehicles = await vehicleModel.find({ Sell: true, }).limit(6).skip(0)
    res.render("All vehicles", { allVehicles: allVehicles, user: user,v:v });
  } else {
    var v = 'buy';
    let allVehicles = await vehicleModel.find({ Sell: true, }).limit(6).skip(0)
    res.render("sell vehicles", { allVehicles: allVehicles, user: null,v:v });
  }

});

router.get("/buycars", async function (req, res, next) {
  if (req.isAuthenticated()) {
    var v = 'buy';
    let user = await userModel.findOne({ email: req.user.email });
    let allVehicles = await vehicleModel.find({ Sell: true,vehicleType: { $regex: /^car$/i } }).limit(6).skip(0)
    res.render("All vehicles", { allVehicles: allVehicles, user: user,v:v });
  } else {
    var v = 'buy';
    let allVehicles = await vehicleModel.find({ Sell: true,vehicleType: { $regex: /^car$/i } }).limit(6).skip(0)
...
router.get("/buybikes", async function (req, res, next) {
  if (req.isAuthenticated()) {
    var v = 'buy';
    let user = await userModel.findOne({ email: req.user.email });
    let allVehicles = await vehicleModel.find({ Sell: true,vehicleType: { $regex: /^bike$/i } }).limit(6).skip(0)
    res.render("All vehicles", { allVehicles: allVehicles, user: user,v:v });
  } else {
    var v = 'buy';
    let allVehicles = await vehicleModel.find({ Sell: true,vehicleType: { $regex: /^bike$/i } }).limit(6).skip(0)
